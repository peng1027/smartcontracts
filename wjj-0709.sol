...
contract WJJDividendTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SignedSafeMath for int256;
    ...
    constructor() DividendPayingToken("WJJ_Dividend_Tracker", "WJJ_Dividend_Tracker") {
        claimWait = 3600;
        minimumTokenBalanceForDividends = 10000 * (10**5); //must hold 10000+ tokens
    }

    function _transfer(address, address, uint256) internal pure override {
        require(false, "WJJ_Dividend_Tracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "WJJ_Dividend_Tracker: withdrawDividend disabled. Use the 'claim' function on the main WJJ contract.");
    }
    ...
    function updateClaimWait(uint256 newClaimWait) external onlyOwner {
        require(newClaimWait >= 3600 && newClaimWait <= 86400, "WJJ_Dividend_Tracker: claimWait must be updated to between 1 and 24 hours");	
        require(newClaimWait != claimWait, "WJJ_Dividend_Tracker: Cannot update claimWait to same value");
...
contract WJJ is ERC20, Ownable, SafeToken, LockToken {
    using SafeMath for uint256;
    ...
    WJJDividendTracker public dividendTracker;
    ...
    constructor() ERC20("WJJ", "WJJ") {  
        ETHRewardFee = 5;
        liquidityFee = 3;
        extraFeeOnSell = 0; // extra fee on sell
        marketingFee = 5;
        marketingWallet = payable(0x87e2da4Fe80EcA3eF52ae2D365462B837F3CD059); 

        totalFees = ETHRewardFee.add(liquidityFee).add(marketingFee); // total fee transfer and buy

    	dividendTracker = new WJJDividendTracker();
    ...
    function updateUniswapV2Router(address newAddress) public onlyOwner {
        require(newAddress != address(uniswapV2Router), "WJJ: The router already has that address");
        emit UpdateUniswapV2Router(newAddress, address(uniswapV2Router));
        uniswapV2Router = IUniswapV2Router02(newAddress);
    }

    function excludeFromFees(address account, bool excluded) public onlyOwner {
        require(_isExcludedFromFees[account] != excluded, "WJJ: Account is already the value of 'excluded'");
    ...
    function setAutomatedMarketMakerPair(address pair, bool value) public onlyOwner {
        require(pair != uniswapV2Pair, "WJJ: The PancakeSwap pair cannot be removed from automatedMarketMakerPairs");
    ...
    function _setAutomatedMarketMakerPair(address pair, bool value) private {
        require(automatedMarketMakerPairs[pair] != value, "WJJ: Automated market maker pair is already set to that value");
    ...
    function updateGasForProcessing(uint256 newValue) public onlyOwner {
        require(newValue >= 200000 && newValue <= 500000, "WJJ: gasForProcessing must be between 200,000 and 500,000");
        require(newValue != gasForProcessing, "WJJ: Cannot update gasForProcessing to same value");
