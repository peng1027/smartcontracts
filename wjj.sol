...
contract WJJToken is IERC20, Ownable {
    ...
    uint256 public _marketFeeBuy = 10;

    uint256 public _marketFeeSell = 20;
    ...
    uint256 private SELLMAX = 100;
    
    address public _marketAddress= 
        address(0x87e2da4Fe80EcA3eF52ae2D365462B837F3CD059);
    ...
    function _transfer(
		...
		if(sender == uniswapV2Pair) {
            ...
            if(!_isExcludedFromFee[recipient]){
                require(tAmount <= IERC20(this).balanceOf(sender).div(100).mul(SELLMAX), "sell must limit lower 90 percent");
            }
            _tokenSellTransferFee(sender, recipient, tAmount);
        } else{
            _tokenTransferFee(sender, recipient, tAmount);
		...
		if(fee){
			...
			uint256 allFee;
			 ...
			 _takeTransfer(
    			sender,
    			_marketAddress,
    			tAmount.div(percent).mul(_marketFeeBuy),
    			currentRate
    			);
            allFee = allFee + _marketFeeBuy;
		...
		if(fee){
            uint256 allFee;
			 _takeTransfer(
    			sender,
    			_marketAddress,
    			tAmount.div(percent).mul(_marketFeeSell),
    			currentRate
    			);
            allFee = allFee + _marketFeeSell;
